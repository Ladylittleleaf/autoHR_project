...
def add_case(caseName="testcase", rule="00*.py"):
    '''第一步：加载所有的测试用例'''
...
def send_mail(sender, psw, receiver, smtpserver, report_file, port1=465,port2=25):
    '''第四步：发送最新的测试报告内容'''
    ...
    msg.attach(att)
    try:
        smtp = smtplib.SMTP_SSL(smtpserver, port1)
    except:
        smtp = smtplib.SMTP()
        smtp.connect(smtpserver,port2)
    # 用户名密码
